...
export function FacultyDashboard() {
  ...
  const loadRequests = useCallback(() => {
    setLoading(true);
    bookingApi.getPendingForFaculty()
  ...
  const handleApprove = async (id) => {
    setActionLoading(id);
    try {
      await bookingApi.approveBooking(id, { remarks: 'Verified' });
      showAlert('Request verified and forwarded to staff', 'success');
      loadRequests();
    } catch (err) {
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Review and verify student booking requests"
      />
            ...
            {requests.length === 0 ? (
              ...
              requests.map((r) => (
                <TableRow key={r.id} hover>
                  ...
                  <TableCell>
                    <Chip label={r.event_type} size="small" sx={{ textTransform: 'capitalize', fontSize: '0.7rem' }} />
                  </TableCell>
                  <TableCell>{r.expected_attendees || '—'}</TableCell>
